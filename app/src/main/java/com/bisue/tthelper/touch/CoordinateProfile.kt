...
/**
 * 상대 좌표 정의용 경량 데이터 클래스 (순수 코틀린 객체로 JVM 유닛테스트 완벽 지원)
 */
data class RelativePoint(val x: Float, val y: Float)
...
object CoordinateProfile {
    ...
    val BTN_LEVELUP_MULTIPLIER = RelativePoint(0.86f, 0.475f)

    // 소드마스터 탭 레벨업 버튼 (소드마스터 초상화 우측)
    val BTN_SWORDMASTER_UPGRADE = RelativePoint(0.82f, 0.54f)

    // 4. 스킬 해금(1레벨) 버튼 목록 (소드마스터 탭 내부)
    // 상단 4개 스킬 (스크롤 전)
    val BTN_SKILL_1_HEAVENLY = RelativePoint(0.82f, 0.635f)
    val BTN_SKILL_2_DEADLY = RelativePoint(0.82f, 0.715f)
    val BTN_SKILL_3_MIDAS = RelativePoint(0.82f, 0.795f)
    val BTN_SKILL_4_FIRESWORD = RelativePoint(0.82f, 0.875f)

    // 하단 2개 스킬 (살짝 스크롤 후 노출)
    val BTN_SKILL_5_WARCRY = RelativePoint(0.82f, 0.70f)
    val BTN_SKILL_6_CLONE = RelativePoint(0.82f, 0.78f)

    // 스킬 목록 아래로 살짝 스크롤하기 위한 제스처 좌표
    val SCROLL_SKILL_START = RelativePoint(0.50f, 0.85f)
    val SCROLL_SKILL_END = RelativePoint(0.50f, 0.60f)

    // 5. 환생(Prestige) 관련 제스처 및 버튼
    // 소드마스터 탭 최하단으로 크게 스크롤
    val SCROLL_PRESTIGE_START = RelativePoint(0.50f, 0.85f)
    val SCROLL_PRESTIGE_END = RelativePoint(0.50f, 0.35f)

    // 환생(Prestige) 진입 버튼 (최하단 스크롤 후)
    val BTN_PRESTIGE = RelativePoint(0.50f, 0.86f)

    // 환생 확인 팝업 승인 버튼
    val BTN_CONFIRM_PRESTIGE = RelativePoint(0.50f, 0.68f)

    // 6. 전투 화면 하단 6개 액티브 스킬 슬롯 (1번 ~ 6번)
    val ACTIVE_SKILL_SLOTS = listOf(
        RelativePoint(0.12f, 0.825f), // 1. 천상의 일격
        RelativePoint(0.27f, 0.825f), // 2. 치명적인 일격
        RelativePoint(0.42f, 0.825f), // 3. 마이더스의 손
        RelativePoint(0.58f, 0.825f), // 4. 불타는 검
        RelativePoint(0.73f, 0.825f), // 5. 함성
        RelativePoint(0.88f, 0.825f)  // 6. 그림자 분신
    )

    // 7. 하단 메뉴 닫기 / 전투 화면 포커스 (화면 중앙 상단 터치)
    val CLOSE_TAB_TAP = RelativePoint(0.50f, 0.25f)

    /**
     * 상대 비율 좌표를 현재 기기의 실제 픽셀 좌표로 변환
     */
    fun toPixel(point: RelativePoint, screenWidth: Int, screenHeight: Int): Pair<Float, Float> {
        return Pair(point.x * screenWidth, point.y * screenHeight)
    }

    fun toPixelBounds(rect: RelativeRect, screenWidth: Int, screenHeight: Int): IntArray {
        return intArrayOf(
